...
#     return output
def quantize(
    input: Tensor,  # [B, N, 8, 8]
    jpeg_quality: Tensor,  # [B]
    quantization_table: Tensor,  # [8, 8]
    rounding_function: Callable[[Tensor], Tensor],
    floor_function: Callable[[Tensor], Tensor],
    clipping_function: Callable[[Tensor, int, int], Tensor],
) -> Tensor:
    """Robust quantize with explicit shape handling.
    ...
    # ensure QT on correct device/dtype and shape [8,8]
    qt = quantization_table.to(dtype).to(device).view(8, 8)  # [8,8]

    # --- build per-batch quant table with explicit shapes ---
    # jpeg_quality_to_scale -> should return shape [B] (or [B,1,1,1]); force to [B]
    scale = jpeg_quality_to_scale(jpeg_quality, floor_function=floor_function)
    scale = scale.view(B)  # ensure shape [B]

    # Expand to [B, 1, 8, 8] (1 is for channel-like dim to align with [B, N, 8, 8])
    quant_table_scaled = qt[None, None, :, :] * scale.view(B, 1, 1, 1)  # -> [B, 1, 8, 8]

    # Apply floor/clipping elementwise (results keep shape [B,1,8,8])
    quant_table = (quant_table_scaled + 50.0) / 100.0
    quant_table = floor_function(quant_table)
    quant_table = clipping_function(quant_table, 1, 255)  # [B,1,8,8]

    # --- now do division with explicit alignment ---
    # input: [B, N, 8, 8], quant_table: [B, 1, 8, 8]
    # Expand quant_table along N dimension explicitly for clarity (optional)
    # quant_table_for_div = quant_table[:, None, :, :]  # shape [B,1,8,8] -> broadcastable to [B,N,8,8]
    output = input / quant_table  # broadcasting: [B,N,8,8] / [B,1,8,8] -> [B,N,8,8]
...
def jpeg_encode(
    ...
    # print("-3. Shape of y_encoded: ", input_y.shape)
    ...
    # print("-2. Shape of y_encoded: ", input_y.shape)
    ...
    # print("-1. Shape of y_encoded: ", y_encoded.shape)
