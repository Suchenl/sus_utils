...
def dequantize(
    ...
    Returns:
        output (Tensor): Dequantized tensor of the shape [B, N, 8, 8].
    """
    ...
    scale = scale.view(B)                                     # [B]

    # explicit broadcast: [B,1,8,8]
    quant_table_scaled = qt[None, None, :, :] * scale.view(B, 1, 1, 1)

    # floor + clip keep shape [B,1,8,8]
    quant_table = (quant_table_scaled + 50.0) / 100.0
    quant_table = floor_function(quant_table)
    quant_table = clipping_function(quant_table, 1, 255)

    # multiply, broadcast along N
    output = input * quant_table  # [B,N,8,8] / [B,1,8,8] -> [B,N,8,8]
    return output
...
def jpeg_decode(
    ...
    # print("idct_y shape", idct_y.shape)
    # Reverse patching
